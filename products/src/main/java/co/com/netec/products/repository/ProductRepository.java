...
public interface ProductRepository extends JpaRepository<Product, String> {
        ...
        @Procedure(name = "Product.listar_nombres_productos")
        String llamarSpListarNombres();

        @Query("SELECT p FROM Product p WHERE LOWER(p.name) LIKE LOWER(CONCAT('%', :keyword, '%'))")
        List<Product> findByNameContainingKeyword(@Param("keyword") String keyword);

        @Query(value = "SELECT * FROM PRODUCTS WHERE PRODUCT_PRICE >= :minPrice", countQuery = "SELECT COUNT(*) FROM PRODUCTS WHERE PRODUCT_PRICE >= :minPrice", nativeQuery = true)
        ...
        @Query(value = "INSERT INTO PRODUCTS (PRODUCT_ID, PRODUCT_CODE, PRODUCT_NAME, PRODUCT_DESCRIPTION, PRODUCT_PRICE) "
                        +
                        "VALUES (:id, :code, :name, :description, :price)", nativeQuery = true)
